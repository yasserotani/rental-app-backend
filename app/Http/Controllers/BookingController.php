...
class BookingController extends Controller
{
    ...
    {
        $booking = Booking::with('apartment.images', 'user')->findOrFail($id);
    ...
    public function getAllUserBookings()
    {
        $userBookings = Auth::user()
            ->bookings()
            ->with('apartment.images', 'user')
            ->orderBy('created_at', 'desc')
            ->paginate(10);

        $mapped = $userBookings->getCollection()->map(function ($booking) {
            return (new BookingResource($booking))->toArray(request());
        });
        $userBookings->setCollection($mapped);
        return response()->json([
    ...
    public function getAllOwnerBookings()
    {
        ...
        $mapped = $ownerBookings->getCollection()->map(function ($booking) {
            return (new BookingResource($booking))->toArray(request());
        });
        $ownerBookings->setCollection($mapped);
        return response()->json([
